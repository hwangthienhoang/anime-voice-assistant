...
const { container, status, message, cue, visibleCue, typing, sequence, progress, availableClips, name, load, playSequence, playClip, stopSequence, zoomStep } = useAvatar();
...
const moments = [
  { clip: 'happy', label: 'Vui vẻ', emotion: 'happy' },
  { clip: 'raise-hand', label: 'Chào bạn', emotion: 'happy' },
  { clip: 'nod', label: 'Gật đầu', emotion: 'relaxed' },
  { clip: 'think', label: 'Suy nghĩ', emotion: 'neutral' },
  { clip: 'relaxed', label: 'Thư giãn', emotion: 'relaxed' },
];
</script>
...
<template>
  <div class="stage-page">
    <header class="stage-page-header">
      <div>
        <p class="stage-eyebrow">WISHLIGHT / CHARACTER STAGE</p>
    ...
    <section class="stage-console" aria-label="Điều khiển animation">
      <div class="console-title">
        ...
        <div><h2>Khoảnh khắc chuyển động</h2><p>Phát chuỗi demo hoặc chọn một cử chỉ để xem riêng.</p></div>
      </div>
      ...
      <div class="console-moments" role="group" aria-label="Chọn cử chỉ">
        <button v-for="moment in moments" :key="moment.clip" type="button" :disabled="status !== 'ready' || !availableClips.includes(moment.clip)" @click="playClip(moment.clip, moment.emotion)">{{ moment.label }}</button>
      </div>
    </section>
...
.stage-page-header { display: flex; align-items: end; justify-content: space-between; gap: 20px; margin-bottom: 20px; }
...
.stage-presentation { position: relative; min-height: 460px; height: min(57vh, 650px); overflow: hidden; border-radius: 18px; box-shadow: var(--shadow-stage); isolation: isolate; }
...
.stage-console { position: relative; display: flex; flex-wrap: wrap; align-items: center; gap: 16px 20px; margin-top: 16px; padding: 18px 20px; border: 1px solid var(--line); border-radius: 14px; background: var(--parchment-raised); box-shadow: var(--shadow-card); }
...
.console-moments { display: flex; flex-wrap: wrap; gap: 7px; width: 100%; padding-top: 10px; border-top: 1px solid var(--line); }.console-moments button { min-height: 32px; padding: 5px 12px; border: 1px solid var(--line); border-radius: 99px; color: var(--ink); background: var(--parchment); font-size: 11px; font-weight: 700; cursor: pointer; }.console-moments button:hover:not(:disabled) { border-color: var(--gold-strong); color: var(--gold-strong); }.console-moments button:disabled { opacity: .45; cursor: not-allowed; }
